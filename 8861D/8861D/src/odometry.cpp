...
float currentAbsoluteOrientation = M_PI;
float prevTheta = M_PI;
...
int positionTracking() {  // COULD TRY PID WITH VOLTAGE INSTEAD
  while (true) {
    ...
    //Else, caluclate the new local position
      ...
      deltaLocalY = 2 * sin(deltaTheta / 2.0) * ((deltaFwdDist / deltaTheta) + 2.50); //-1.29 for 180 degree turn // -1.50 for 90 degree turn //Measure Forward Tracking wheel Track Radius
      deltaLocalX = 2 * sin(deltaTheta / 2.0) * ((deltaSdwDist / deltaTheta) + 7); //-4.14 measured for 360 degree turn // -3.89 measured for 90 degree turn //check if deltaSdwdist is negative when deltaTheta is positive, if so use + otherwise use - // before was + 0.065 for best results //Measure Sideways Track Radius
