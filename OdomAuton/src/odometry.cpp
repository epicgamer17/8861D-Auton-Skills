...
int positionTracking() {  // COULD TRY PID WITH VOLTAGE INSTEAD
  while (true) {
    ...
    //Else, caluclate the new local position
      ...
      deltaLocalY = 2 * sin(deltaTheta / 2.0) * ((deltaFwdDist / deltaTheta) + 0.125); //Measure Forward Tracking wheel Track Radius
      deltaLocalX = 2 * sin(deltaTheta / 2.0) * ((deltaSdwDist / deltaTheta) - 0.0725); //Measure Sideways Track Radius
