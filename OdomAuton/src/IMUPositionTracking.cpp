...
int IMUpositionTracking() {  // COULD TRY PID WITH VOLTAGE INSTEAD
  while (true) {
    ...
    // fwdSpeed += inertialSensor.acceleration(xaxis) * sin(currentAbsoluteOrientation)  + inertialSensor.acceleration(yaxis) * cos(currentAbsoluteOrientation);
    // IMUGlobalY += fwdSpeed;
    
    // sdwSpeed += inertialSensor.acceleration(yaxis) * sin(currentAbsoluteOrientation) + inertialSensor.acceleration(xaxis) * cos(currentAbsoluteOrientation);
    // IMUGlobalX += sdwSpeed;
