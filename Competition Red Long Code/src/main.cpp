...
void goToFunnel() {
  ...
  toggleFlyWheel();
  driveTo(1.76, 0.40, 0, 3000, 1, false);
  waitUntil(enablePID == false);
  correctPosition(0);
  waitUntil(enablePID == false);
...
void goToNearestRoller() {
  if (ptToPtDistance(globalX, globalY, 0.525, 0.525) < ptToPtDistance(globalX, globalY, 3.075, 3.075)) {  
    if (ptToPtDistance(globalX, globalY, 0.15, 0.735) < ptToPtDistance(globalX, globalY, 0.735, 0.15)) {
      ...
      driveTo(0.75, 0.3, M_PI/2, 3000, 1, false);
      waitUntil(enablePID == false);
      correctPosition(90);
    ...
    if (ptToPtDistance(globalX, globalY, 2.9, 3.45) < ptToPtDistance(globalX, globalY, 3.45, 2.9)) {
      driveTo(2.9, 3.25, 3*M_PI/2, 3000, 1, false);
      waitUntil(enablePID == false);
      correctPosition(270);
      ...
      driveTo(3.25, 2.9, M_PI, 3000, 1, false);
      waitUntil(enablePID == false);
      correctPosition(180);
...
void competitionAutonomous() {
  ...
  //Shoot in High Goal
  setFlyWheelSpeed(1.2, 0.6);
  toggleFlyWheel();
  driveTo(1.2, 0.6, M_PI/2, 1500, 1, false);
  waitUntil(enablePID == false);
  ...
  //Three Stack
  driveTo(1.5, 0.9, 5*M_PI/4, 2250, 0.75, false); //0.31 before
  waitUntil(enablePID == false);
  
  setFlyWheelSpeed(1.85, 1.25);
  toggleFlyWheel();
  
  driveTo(1.85, 1.25, 5*M_PI/4, 1500, 1, false); //0.31 before
  waitUntil(enablePID == false);
  ...
  //Three Extra Discs 
  toggleFlyWheel();
  driveTo(3, 2.4, 5*M_PI/4, 1500, 1, false); //0.31 before
  waitUntil(enablePID == false);
  ...
  turnToPoint(0.45, 3.16, 2000, false, false); //3.3, 0.35 //3.15, 0.35
  waitUntil(enablePID == false);
  ...
  // //Second Roller//
  // driveTo(3.25, 2.85, M_PI, 1000, 1, false); //0.31 before
  // waitUntil(enablePID == false);
...
void autonomous(void) {  
  ...
  correctPosition(90); //set start position
  // autonSkillsV1();
  // autonSkillsV2();
